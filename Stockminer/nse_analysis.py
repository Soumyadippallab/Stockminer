...
def analyze_nse_stocks(num_tickers=None, top_x=None, start_date=None, end_date=None):
    ...
    if summary_list:
        ...
        if 'Sector' in final_summary.columns:
            ...
            plt.figure(figsize=(12, 6))
            sector_cagr = final_summary.groupby('Sector')['CAGR_%'].mean().sort_values(ascending=False)
            sns.barplot(x=sector_cagr.index, y=sector_cagr.values)
            plt.title('Average CAGR by Sector')
            plt.xlabel('Sector')
        ...
        # ANOVA test
        if len(final_summary['Sector'].unique()) > 1:
            sector_returns = [group['Avg_Return_%'].values for name, group in final_summary.groupby('Sector')]
            f_stat, p_value = stats.f_oneway(*sector_returns)
            print(f"\n📋 ANOVA Test for Returns Across Sectors:")
            print(f"F-statistic: {f_stat:.4f}, P-value: {p_value:.4f}")
            if p_value < 0.05:
                print(" ✓ Significant differences in returns across sectors")
        ...
        return final_summary, sector_stats, overall_stats, corr_matrix, top_cagr, top_sharpe
